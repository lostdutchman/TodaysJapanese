/*To use this code follow the Amazon Alexa Facts skill walkthrough and replace the Lambda script with this. Sorry if the codes a bit lazy, we were in a ruch to complete it and since this venture made $0 we had no reason to spend the time cleaning it up. */

'use strict';

const Alexa = require('alexa-sdk');

const APP_ID = "amzn1.ask.skill.XXX.XXX.XXX.XXX.XXXXXXX";  //replace with your app ID.
...
const languageStrings = {
    'en': {
        translation: {
            ...
            SKILL_NAME: 'Todays Japanese',
            GET_FACT_MESSAGE: "Todays word is: ",
            HELP_MESSAGE: 'You can say what is the word of the day, or, you can say exit... What can I help you with?',
...
const handlers = {
    'LaunchRequest': function () {
        this.emit('TodaysJapanese');
    },
    'todays_word': function () {
        this.emit('TodaysJapanese');
    },
    'TodaysJapanese': function () {
        // Get a random space fact from the space facts list
        // Use this.t() to get corresponding language data
        const factArr = this.t('FACTS');
    var today = new Date().setHours(0,0,0,0);//remove hours, min, seconds, and milliseconds
    const factIndex = Math.floor(today / 86400000) % 100; // devide by number of milliseconds in a day then modulus the number of words we have.
